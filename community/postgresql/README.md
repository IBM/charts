...
```console
$ helm install community/postgresql
```
...
You can provide an existing secret containing `postgresql-password` and if replication is enabled `postgresql-replication-password`.
The existing secret can be provided `global.postgresql.existingSecret` or `postgresql.existingSecret` depending upon usecase.
...
kubectl create secret generic my-release-<secret_name> \
  --from-literal='postgresql-password=XXXX' \
  --from-literal='postgresql-replication-password=XXXX' \
  --namespace namespace_name
```

### Image Security Policies

If the cluster has image security policies enforced, postgressql docker images should be added to it.
...
If `volumePermissions.enabled` is set to `true` then an initContainer will be used to set up volume permissions for nonroot user.

If `default` namespace is used to deploy the chart then no action is needed as PSP for the namespace allows containers to be run as root.

If the intent is to use a `non-default`namespace, then you have one of the below options.
- [Recommended] Set up volume permissions separate from the chart and set `volumePermissions.enabled` to `false`. This will allow you to deploy chart with nonroot user.
- If `volumePermissions.enabled` is set to `true`, then use `ibm-anyuid-hostpath-psp` which is less restrictive and allows containers to be run as root.
- If `volumePermissions.enabled` is set to `true`, then have your cluster administrator create a custom PodSecurityPolicy for non-default namespace to allow containers to be run as root. Refer [Creating a custom pod security policy](https://www.ibm.com/support/knowledgecenter/SSBS6K_3.1.2/user_management/custom_psp.html)

## Installing the Chart
To install the chart with the release name `my-release`:

```console
$ helm install --name my-release community/postgresql
```
...
If secret is created by chart, you must
...
| `persistence.useDynamicProvisioning`          | Enable dyanamic Provisioning     | `false` |
...
```console
$ helm install --name my-release -f values.yaml community/postgresql
```
...
```console
$ helm install --name my-release -f ./values-production.yaml community/postgresql
$ kubectl scale statefulset my-postgresql-slave --replicas=3
```

## Persistence

The [Bitnami PostgreSQL](https://github.com/bitnami/bitnami-docker-postgresql) image stores the PostgreSQL data and configurations at the `Values.persistence.mountPath` path of the container.
...
```
## Support
PostgreSQL has a wide variety of community and commercial support options available for users. Visit [Postgresql Support Page](https://www.postgresql.org/support/) for details.
