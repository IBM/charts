...
*/}}
{{- define "ibm-minio.affinityMinio" -}}
  {{- if .Values.affinityMinio -}}
    {{- if kindIs "string" .Values.affinityMinio -}}
      {{- tpl .Values.affinityMinio . -}}
    {{- else -}}
      {{- $root := . -}}
      {{- range $key, $value := .Values.affinityMinio }}
{{ tpl $value $root }}
      {{- end }}
    {{- end -}}
