...
#sed -i 's/10.0.0.2/10.0.0.5-x86_64/g' $preinstallDir/../values.yaml

echo "Prepare a base directory for Persistent Volumes"
nfsRoot="${preinstallDir}/../pv-root"
mkdir -p "${nfsRoot}"

nfsRootvmoc="${preinstallDir}/../pv-root-vmoc"
mkdir -p "${nfsRootvmoc}"
...
sudo mount -t nfs "${nfsServer}:/NFSSHARE" "${nfsRoot}"

sudo mount -t nfs "${nfsServer}:/shared/nfs" "${nfsRootvmoc}"
sudo chown -R 1001 "${nfsRootvmoc}/"
sudo chgrp -R root "${nfsRootvmoc}/"
sudo chmod -R 777 "${nfsRootvmoc}/"
