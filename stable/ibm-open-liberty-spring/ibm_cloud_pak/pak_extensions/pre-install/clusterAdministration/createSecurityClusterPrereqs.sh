...
if supports_scc; then 
  ...
  kubectl apply -f ibm-open-liberty-spring-scc.yaml --validate=false
fi

if supports_psp; then 
  # Create the PodSecurityPolicy and ClusterRole for all releases of this chart.
  echo "Creating the PodSecurityPolicy..."
  kubectl apply -f ibm-open-liberty-spring-psp.yaml
  kubectl apply -f ibm-open-liberty-spring-cr.yaml
fi