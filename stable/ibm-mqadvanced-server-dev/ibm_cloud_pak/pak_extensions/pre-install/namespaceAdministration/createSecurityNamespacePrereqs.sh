...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    ...
    oc adm policy add-scc-to-group ibm-mq-dev-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-mq-dev-rb.yaml > $namespace-ibm-mq-dev-rb.yaml
  ...
  kubectl create -f $namespace-ibm-mq-dev-rb.yaml -n $namespace
