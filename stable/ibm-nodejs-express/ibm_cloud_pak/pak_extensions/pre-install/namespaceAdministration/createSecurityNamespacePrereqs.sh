...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group ibm-nodejs-express-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-nodejs-express-rb.yaml > $namespace-ibm-nodejs-express-rb.yaml
  ...
  kubectl create -f $namespace-ibm-nodejs-express-rb.yaml -n $namespace
