...
cat <<-USAGE #| fmt
        ...
        OPTIONS:
        =======
        --namespace        [namespace]           - The name of the namespace for the CEM deployment.
USAGE
}
...
if [ `kubectl get namespace | grep $NAMESPACE | wc -l` -gt 0 ]; then
   ...
   echo "Creating namespace $NAMESPACE for CEM"
   kubectl create -f namespace.yaml
fi

echo "Creating Security Context Constraints for CEM"
