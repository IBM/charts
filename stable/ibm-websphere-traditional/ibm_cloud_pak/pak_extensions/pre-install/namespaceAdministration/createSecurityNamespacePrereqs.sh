...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group ibm-websphere-traditional-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-websphere-traditional-rb.yaml > $namespace-ibm-websphere-traditional-rb.yaml
  ...
  kubectl create -f $namespace-ibm-websphere-traditional-rb.yaml -n $namespace
