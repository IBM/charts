...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group db2wh-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed "s/%NAMESPACE%/$namespace/g" "${DIR}/ibm-db2wh-prod-rb.yaml" > "${DIR}/${namespace}-ibm-db2wh-prod-rb.yaml"
  ...
  cat "${DIR}/${namespace}-ibm-db2wh-prod-rb.yaml"
  echo ""
  kubectl create -f "${DIR}/${namespace}-ibm-db2wh-prod-rb.yaml" -n $namespace

  # Clean up - delete the temporary yaml file.
  rm "${DIR}/${namespace}-ibm-db2wh-prod-rb.yaml"
fi;