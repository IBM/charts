...
Before creating a pull request to add a chart to the IBM Community charts repository, chart owners must verify that the chart deploys as expected on the latest version of IBM Cloud Private, using both the IBM Cloud Private user interface and the Helm command line.  In addition, if there are any versions of IBM Cloud Private known to not work with the chart, those limitations should be clearly specified in the README.md.  For example: `This chart is only supported on IBM Cloud Private version 3.1.0 and above.`
