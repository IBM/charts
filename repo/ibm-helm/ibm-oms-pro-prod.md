...
- Ensure that the MQ (or other JMS) server is installed and the MQ server is accessible from inside the cluster.
...
```sh
helm install my-release -f values.yaml ./ibm-oms-pro-prod --timeout 3600 --namespace <namespace>
```
...
```sh
helm upgrade my-release -f values.yaml ./ibm-oms-pro-prod --timeout 3600
```
