## Introduction
Built on the open-source [Apache Flink](https://flink.apache.org), IBM OpenContent Flink Operator acts
as a control plane to manage the complete deployment lifecycle of Flink applications.

## Chart Details
This chart can be used to install IBM OpenContent Flink Operator, which has the following key features:
...
### Resources Required
IBM OpenContent Flink Operator resource requirements depend on several factors. For information about minimum
resource requirements, see the [documentation](https://ibm.biz/event-automation).

## Installing the Chart
For information about installing the IBM OpenContent Flink Operator, see the [documentation](https://ibm.biz/ep-installing-flink).
...
## Configuration
IBM OpenContent Flink Operator provides sample configurations to help you get started with deployments.
...
Note that the fields `image` and `flinkVersion` should not be set, as they are controlled by the IBM OpenContent Flink Operator.
