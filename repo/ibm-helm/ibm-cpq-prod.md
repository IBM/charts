# IBM Sterling Configure Price Quote Software v10.0.0.6

## What's New
* The v10.0.0.6 release of CPQ is built and deployed on Openshift 4.4.
...
* Swagger API Tester for Configurator, Search below for more details.

## Introduction

This document describes how to deploy IBM Sterling Configure Price Quote Software v10.0.0.6. This helm chart does not install database server. Database need to be setup and configured separately for IBM Sterling Configure Price Quote Software .
...
6. Push the image to Openshift registry  
   ...
   * Once you login to the registry, you will need to tag it appropriately.
     ...
     `podman tag [ImageId] [image-registry-routeURL]/default/cpq-vm-app:10.0.0.6-x86_64`
      Then push the image to this registry using 
     `podman push [image-registry-routeURL]/default/cpq-vm-app:10.0.0.6-x86_64`
...
```
 ...
 4. Set the following variables in the values.yaml
    ...
    2. Set the image names -
        ...
        e.g. vmappserver.image.tag: 10.0.0.6-x86_64
        e.g. ocappserver.image.name: cpq-oc-app
        e.g. ocappserver.image.tag: 10.0.0.6-x86_64
...
`vmdatasetup.loadFactoryData`                  | Load factory data of IFS Application                                 |
...
# IBM Sterling Field Sales Edition v10.0.0.3
=======================================================================

## Introduction

The below content describes how to deploy IBM Sterling Field Sales v10.0.0.3. This helm chart does not install database server or messaging server. Both these middlewares need to be setup and configured separately for IBM Sterling Field Sales.
...
4. Set the following variables in the values.yaml
    ...
    2. Set the image names -
        ...
        e.g. ifsappserver.image.tag: 10.0.0.3-x86_64
        e.g. ifsagentserver.image.name: cpq-ifs-agent
        e.g. ifsagentserver.image.tag: 10.0.0.3-x86_64
    3. Set the ingress host
...
* Configurtor comes with a feature for testing it's Rest API via Swagger.
