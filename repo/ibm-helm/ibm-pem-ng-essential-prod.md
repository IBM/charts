...
## Configuration
The following table lists the configurable parameters of the ibm-pem-ng-essential chart and their default values.

| Parameter                | Description             | Default        |
| ------------------------ | ----------------------- | -------------- |
| `image.name` | Provide the value in double quotes | `"cp.icr.io/cp/ibm-pem-ng/pemng"` |
| `image.tag` | Specify the tag name | `"6.3.0.3"` |
...
`identityService.image.tag          `                   | Docker image tag                                                     | `1.0.1.3`
...
```sh
$ helm install my-release -f values.yaml ./ibm-pem-ng-essential --timeout 3600s  --namespace <namespace>
```
...
```sh
helm upgrade my-release -f values.yaml ./ibm-pem-ng-essential --timeout 3600s
```

4. Run the following command to upgrade your deployments:
```
helm upgrade my-release -f values.yaml ./ibm-pem-ng-essential --timeout 3600s --recreate-pods
```
