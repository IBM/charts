...
## Prerequisites
  ...
  4. For installation instructions and requisite requirements, see Installation of DevOps Loop at <https://www.ibm.com/docs/en/devops-loop/1.0.2?topic=guide-installation-devops-loop>
...
Before you begin, follow requisite steps and configuration at Installation of DevOps Loop:  <https://www.ibm.com/docs/en/devops-loop/1.0.2?topic=guide-installation-devops-loop>
...
#https://www.ibm.com/docs/en/devops-loop/1.0.2?topic=guide-installation-devops-loop
DOMAIN=
...
"
  ...
  helm upgrade --install ${HELM_NAME} ibm-helm/ibm-devops-loop --version 1.0.2 ${HELM_OPTIONS} \
...
```bash
helm repo add ibm-helm https://raw.githubusercontent.com/IBM/charts/master/repo/ibm-helm --force-update
helm pull --untar ibm-helm/ibm-devops-loop --version 1.0.2
```
...
#for instance devops-loop.$(oc get....), using an altnernative ingress controller, or
...
run_install() { 
  ...
  #Update domain name for Velocity to fix issue in v5.1.6
  ...
  #Remove namespace in Code network-policy.yaml
  if [ -e ${ROOT_DIR}/charts/ibm-devopscode*.tgz ]; then
    cd ${ROOT_DIR}/charts
    tar zxf ibm-devopscode*.tgz
    cd -
  fi
...
"
  ...
  helm upgrade --install ${HELM_NAME} ${ROOT_DIR} --version 1.0.2 ${HELM_OPTIONS} \
