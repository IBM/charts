# IBM Sterling Secure Proxy Configuration Manager v6.0.2.0
...
Before you install IBM Certified Container Software for Sterling Secure Proxy CM, ensure that the helm chart package is available on your client system.

#### Downloading the IBM Certified Container Software helm chart from IBM Chart repository
You can download the IBM CCS for SSP CM helm chart from [IBM Public chart repository](https://www.ibm.com/links?url=https://github.com/IBM/charts/tree/master/repo/ibm-helm/ibm-ssp-cm-1.1.2.tgz).
...
```
cloudctl case save -t 1 --case https://github.com/IBM/cloud-pak/raw/master/repo/case/ibm-ssp-cm/1.0.1/ibm-ssp-cm-1.0.1.tgz --outputdir download_dir/ && tar -xf download_dir/ibm-ssp-cm-1.0.1.tgz
```
> **Note**: download_dir is the output directory in which the IBM Sterling Secure Proxy - CM resources are placed. The output directory is created if it does not exist. You can choose an arbitrary name for --outputdir if required.` 
...
- Pull the container image from IBM Entitled registry by running the following command:
  * `docker/podman pull cp.icr.io/cp/ibm-ssp-cm/ssp-cm-docker-image:6.0.2.0.02`

- Tag and push the container image into local repository by running the following commands: 
  * `docker/podman tag cp.icr.io/cp/ibm-ssp-cm/ssp-cm-docker-image:6.0.2.0.02 <LOCAL_DOCKER_REGISTRY_REPOSITORY>/ssp-cm-docker-image:6.0.2.0.02`
  * `docker/podman push <LOCAL_DOCKER_REGISTRY_REPOSITORY>/ssp-cm-docker-image:6.0.2.0.02`
...
IBM Sterling Secure Proxy - Configuration Manager supports NFS storage for storing the persistent data. This needs to be created as a pre-requisite before deploying IBM Sterling Secure Proxy - Configuration Manager. However, IBM Sterling Secure Proxy - Configuration Manager supports - 
...
- NFS/Hostpath mount path should have custom key store certificate file, if you want to use custom certificate otherwise it's not needed. For this, first create the directory with 'CM_RESOURCES' name inside mount path if not available and place custom key store certificate file inside created directory. So during installation it will update the custom key store cert and export the key certificate file into PV directory(CM) that can be imported as a custom cert into Engine. You have to install CM first and provide the required information to 'customCertificate' tag otherwise it will not work.   
...
The IBM Sterling Secure Proxy CM secret would be created with data described as below: -
...
|`adminPassword`            | The adminPassword is used when logging into CM for the first time.                                                    |
...
|`customKeyCertPassphrase`  | The customKeyCertPassphrase is required if you want to use custom certificate during the installation.                |
...
To map the exported key certificate from Engine using secret. 
```
kubectl create secret generic <KeyCert Secret Name> --from-file=<Key Name>=<Engine Exported Key Certificate File>
```
...
> **Note**: It uses only when you are installing Configuration Manager after Engine otherwise it's not needed. 
...
## Agreement to IBM Sterling Secure Proxy CM License

You must read the IBM Sterling Secure Proxy CM License agreement terms before installation, using the below link:
[License] http://www-03.ibm.com/software/sla/sladb.nsf (L/N: L-BCHE-BXLMPM)

## Installing the Chart

To install IBM Sterling Secure Proxy CM via Helm command line with the release name e.g. `my-release`.
...
$ helm repo add ibm-ssp-cm-1.1.2.tgz <helm repository>
...
$ helm install my-release  --set image.repository=<repo name>,image.tag=<image tag>,image.imageSecrets=<image pull secret>,secret.secretName=<CM secret name> ibm-ssp-cm-1.1.2.tgz
```

The command deploys ibm-ssp-cm-1.1.2.tgz chart on the Kubernetes cluster in the default configuration. The [configuration](#configuration) section lists the parameters that can be configured during installation.

> **Tip**: List all releases using `helm list`

## Configuration

The following tables lists the configurable parameters of the IBM Sterling Secure Proxy CM chart and their default values.
...
| `image.digest.value`                      | Digest has value for image used for deployment      | `sha256:67cb57afe3be3e70eb81604ff82aa6f9498617ea55e0a7da8cb3eb6f55804f82`                                       |
| `image.pullPolicy`                        | Image pull policy                                   | `IfNotPresent`                           |
| `cmArgs.appUserUid`                       | Container user UID                                  | `1000`                                   |
| `cmArgs.appUserGid`                       | Container user GID                                  | `1000`                                   |
| `cmArgs.keyCertExport`                    | Export or Import key certificate                    | `true`                                   |
| `cmArgs.keyCertAliasName`                 | Certificate alias name                              | `keycert`                                |
| `cmArgs.maxHeapSize`                      | JVM heap size                                       | `2048m`                                  |
...
| `service.jetty.servicePort`               | Jetty web service port                              | `8443`                                   |
| `service.jetty.containerPort`             | Jetty web container port                            | `8443`                                   |
| `service.cm.servicePort`                  | CM service port                                     | `62366`                                  |
| `service.cm.containerPort`                | CM container port                                   | `62366`                                  |
...
| `customCertificate.commonCertFile`     | Key store certificate file name                     |                                          |
| `customCertificate.commonCertAlias`    | Alias name                                          |                                          |
| `customCertificate.commonKeyCertFile`  | Export key certificate file name                    |                                          |
...
helm install my-release \
  --set service.jetty.servicePort=8444 \
  ibm-ssp-cm-1.1.2.tgz
```
...
```bash
helm install <release-name> -f my-values.yaml ibm-ssp-cm-1.1.2.tgz
```
...
* Node affinity - This can be configured using parameters `affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution`, `affinity.nodeAffinity.preferredDuringSchedulingIgnoredDuringExecution` for the Sterling Secure Proxy CM server.
...
* Pod affinity - This can be configured using parameters `affinity.podAffinity.requiredDuringSchedulingIgnoredDuringExecution`, `affinity.podAffinity.preferredDuringSchedulingIgnoredDuringExecution` for the Sterling Secure Proxy CM server.

* Pod anti-affinity - This can be configured using parameters `affinity.podAntiAffinity.requiredDuringSchedulingIgnoredDuringExecution`, `affinity.podAntiAffinity.preferredDuringSchedulingIgnoredDuringExecution` for the Sterling Secure Proxy CM server.
...
```bash
helm upgrade my-release -f values.yaml ibm-ssp-cm-1.1.2.tgz
```
...
3. PodSecurityPolicy/SecurityContextConstraints 
...
This chart creates a service of `ClusterIP` for communication within the cluster. This type can be changed while installing chart using `service.type` key defined in values.yaml. There are two ports where IBM Sterling Secure Proxy CM processes run. Web server port (8443) and cm port (62366), whose values can be updated during chart installation using `service.jetty.servicePort` or `service.cm.servicePort`.

IBM Sterling Secure Proxy CM service can be accessed using LoadBalancer external IP and mapped jetty port. If external LoadBalancer is not present then refer to Master node IP for communication.
