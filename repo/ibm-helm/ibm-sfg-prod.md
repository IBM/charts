# IBM Sterling File Gateway Enterprise Edition v6.1.2.8
## Introduction
...
5. Ensure that the docker images for IBM Sterling File Gateway Software Enterprise Edition from Passport Advantage are loaded to an appropriate docker registry.
...
10. Create secrets with requisite confidential credentials for system passphrase, database, MQ server and Liberty. You can use the supplied configuration files under pak_extensions/pre-install/secret directory. 
...
12. If applicable, create secrets with confidential certificates required by Database, MQ or Liberty for SSL connectivity using below command
    ```
	kubectl create secret generic <secret-name> --from-file=/path/to/<certificate>
	```

13. When installing the chart on a new database which does not have IBM Sterling B2B Integrator Software schema tables and metadata, 
...
14. When installing the chart on a database on an older release version
...
`global.image.tag          `                   | Docker image tag                                                     | `6.1.2.8`
...
`purge.image.tag          `                    | External purge image tag                                             | `6.1.2.8`
