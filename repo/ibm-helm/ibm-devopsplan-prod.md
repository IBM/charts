...
[DevOps Plan](https://ibm.com/docs/en/devops-plan/3.0.9) is a change management platform designed for enterprise-level scalability, customizable processes, and enhanced project control. It accelerates project delivery and improves developer productivity..
...
- [DevOps Plan Product Documentation](https://ibm.com/docs/en/devops-plan/3.0.9)
...
**Note:** If the external IP address is **not the same as the Kubernetes node IP**, you must explicitly provide the node’s internal IP address when running `helm install` or `helm upgrade`.
...
```bash
--set postgresql.service.nodeInternalIP=${NODE_IP}
```
...
**Procedure:**
1. Run *helm history* command to see revision numbers of your helm chart release. You should have min two revision numbers. revision 1 for install and revision 2 for the upgrade that you execute in **Before you begin** section. Example below shows you have a helm chart release name *ibm-devopsplan1* with revision 1 installed the helm chart *ibm-devopsplan1-3.0.8* for release 3.0.8 and revision 2 upgraded the helm chart *ibm-devopsplan2-3.0.9* to release 3.0.9.

  ```bash
  $ helm history ibm-devopsplan1 --namespace dev
  REVISION        UPDATED                         STATUS          CHART           APP VERSION     DESCRIPTION
  1               Thu Nov 20 21:58:13 2024        superseded      ibm-devopsplan1-3.0.8          Install complete
  2               Thu Nov 20 22:13:56 2024        deployed        ibm-devopsplan2-3.0.9          Upgrade complete
  ```
...
3. Run *helm history RELEASE* command again to see the new revision 3 has been created after rollback, and it rollbacked to revision 1 the helm chart release *ibm-devopsplan1*.
  ...
  1               Thu Nov 20 21:58:13 2024        superseded      ibm-devopsplan1-3.0.8          Install complete
  2               Thu Nov 20 22:13:56 2024        deployed        ibm-devopsplan2-3.0.9          Upgrade complete
  3               Thu Nov 20 22:30:32 2024        deployed        ibm-devopsplan1-3.0.8          Rollback to 1
  ```

- The OpenSearch version has been upgraded from 2.18.0 to 3.1.0 as part of the DevOps Plan 3.0.7 release. OpenSearch 2.18.0 is based on Lucene 9, whereas OpenSearch 3.1.0 is based on Lucene 10. This Rollback constitutes a major version change; therefore, in-place rolling back are not supported. To complete the rollback, either a full cluster restart or a migration involving reindexing is required. For detailed guidance, refer to the official OpenSearch documentation.
...
| **analytics.image.tag** | Analytics Image tag. This parameter is needed if *analytics.service=true.* | 3.0.9 |
...
| **postgresql.tag** | Postgresql database Image tag.This parameter is needed if *postgresql.enabled=true.* | 3.0.9 |
...
| **nginx.image.tag** | Nginx Image tag. This parameter is needed if *nginx.service=true.* | 3.0.9 |
...
| **dashboards.image.tag** | Opensearch-dashboards Image tag. This parameter is needed if *dashboards.service=true.* | 3.0.9 |
...
| **logstash.image.tag** | devopsplan-logstash Image tag. This parameter is needed if *logstas.service=true.* | 3.0.9 |
...
| **opensearch.image.tag** | Opensearch Image tag. This parameter is needed if *opensearch.service=true.* | 3.0.9 |
...
| **keycloaksrv.image.tag** | Keycloak Image tag. | 3.0.9 |
...
| **winInstall.image.tag** | win-install image tag. This parameter is needed if *winInstall.enabled=*true* | 3.0.9 |
...
- [DevOps Plan](https://ibm.com/docs/en/devops-plan/3.0.9)
- [Getting started with DevOps Plan Helm Chart](https://www.ibm.com/docs/en/devops-plan/3.0.9?topic=plan-getting-started-devops-helm-chart-openshift)
