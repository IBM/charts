...
if supports_psp; then
  ...
  sed 's//'$namespace'/g' "${DIR}/../../pre-install/namespaceAdministration/ibm-db2oltp-prod-rb.yaml" > $namespace-ibm-db2oltp-prod-rb.yaml

  # Delete the role binding for all service accounts in the current namespace
  kubectl delete -f "${DIR}/$namespace-ibm-db2oltp-prod-rb.yaml" -n $namespace
