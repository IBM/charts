...
def purge_active_kernels(kernel_service_url,instance_id,hb_endpoint,purge_time):
    ...
    k_response = requests.get("{}/{}/meta/api/kernels?state=Active".format(kernel_service_url, instance_id), headers=headers, verify=False)
    if k_response.status_code == 200:
        ...
        if len(kernel_list_json) != 0 :
            for kernel in kernel_list_json:
                ...
                state_api_response = requests.get("{}/{}/jkg/api/kernels/{}".format(hb_endpoint, instance_id, kernel_id), headers=headers, verify=False, timeout=5)
...
def purge_failed_kernels(kernel_service_url,instance_id,hb_endpoint):
    response = requests.get("""{}/{}/meta/api/kernels?state=Failed""".format(kernel_service_url, instance_id), headers=headers, verify=False)
