#!/bin/bash
# This script is created based on the build script of the 0040-ws-base (../0040-ws-base/build.sh)
# This script is used to create ibm-volume-setup-prod module which will be installed after the prereqs module by the installer
#
set -e
...
CONFIG_FILE="ibm-volume-setup-prod"
...
if [ $ARCH = "ppc64le" ]; then
    ...
    echo "dsx-requisite||PrivateCloud/dsx-requisite||${dsxcoreBranch}||ibm-private-cloud||DSXL-core-requisite-power||BR:dsxcoreBranch||9.30.4.45||.." >> ${CONFIG_FILE_PATH}
else
    echo "#Service||Repo||Branch||Namespace||Jenkins||Jenkins Branch Name||project build machine||path" > ${CONFIG_FILE_PATH}
    echo "dsx-requisite||PrivateCloud/dsx-requisite||${dsxcoreBranch}||ibm-private-cloud||DSXL-core-requisite||BR:dsxcoreBranch||9.30.4.45||.." >> ${CONFIG_FILE_PATH}
fi
...
if [[ ${PUSH_TO_FILESERVER} = 'TRUE' ]]; then
  ...
  volumesetupFolder="/pool1/data1/zen/cp4d-builds/3.0.1/local/components/common-core-services/modules/volume-setup"
  VersionFileLocation=${volumesetupFolder}/${ARCH}/versions.yaml
  sshpass -p $fileServerPassword scp -r -o StrictHostKeyChecking=no 3.0.${BUILD_NUMBER} build@icpfs1.svl.ibm.com:${volumesetupFolder}/${ARCH}
