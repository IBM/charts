...
*/}}
{{- define "ibm-mongodb.affinity" -}}
  {{- $allParams := . }}
  {{- $root    := first . }}
  {{- $details := first (rest . ) }}
  {{- $_       := set $root "affinityDetails" $details -}}

  {{- if and $root.Values.affinityMongodb (eq $details.component $root.sch.chart.components.server) -}}
    {{- if kindIs "string" $root.Values.affinityMongodb -}}
      {{- tpl $root.Values.affinityMongodb $root -}}
    {{- else -}}
      {{- range $key, $value := $root.Values.affinityMongodb }}
{{ tpl $value $root }}
      {{- end }}
    {{- end -}}
...
{{- define "ibm-mongodb.antiAffinity" -}}
...
podAntiAffinity:
  ...
  - weight: 1
    podAffinityTerm:
      ...
      labelSelector:
        matchLabels:
{{ include "sch.metadata.labels.standard" (list . .sch.chart.components.server) | indent 10 }}
    {{- end }}
  {{- end -}}
{{- end -}}
