...
- name: {{ $name }}
  securityContext:
    ...
    capabilities:
      drop:
      - ALL
  image: "{{ $global.Values.global.dockerRegistryPrefix }}/{{ $global.Values.images.miniomc.image }}:{{ $global.Values.images.miniomc.tag }}"
  resources:
    requests:
      memory: "1024Mi"
      cpu: "1"
...
{{- define "chuck.getAliasedModelsHelper" -}}
{{- $params := . -}}
{{- $models := index $params 0 -}}
{{- $aliases := index $params 1 -}}
{{- range $key, $model := $models -}}
{{- range $akey, $amodels := $aliases -}}
{{- if and (eq $akey $key) (include "chuck.isModelEnabled" $model) -}}
{{- range $alias := $amodels }}
{{ $alias | quote }},
{{- end -}}
{{- end -}}
...
{{- define "chuck.getAliasedModels" -}}
{{- $result := trim (trimSuffix "," (include "chuck.getAliasedModelsHelper" . )) -}}
{{- if ne $result "" -}}
,
{{ $result | indent 15}}
{{- end -}}
...
{{- $maxSession := div (mul 1000 $cpus) $resources.sessionCPUFactor }}
