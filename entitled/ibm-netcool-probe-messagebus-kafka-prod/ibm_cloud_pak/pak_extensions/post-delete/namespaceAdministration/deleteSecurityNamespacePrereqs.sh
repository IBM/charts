...
if supports_scc; then
  ...
  oc adm policy remove-scc-from-group ibm-netcool-probe-messagebus-kafka-prod-scc system:serviceaccounts:$namespace
elif supports_psp; then
  # Replace the NAMESPACE tag with the namespace specified in a temporary yaml file.
  sed 's/{{ NAMESPACE }}/'$namespace'/g' $scriptDir/../../pre-install/namespaceAdministration/ibm-netcool-probe-messagebus-kafka-prod-rb.yaml > $scriptDir/$namespace-ibm-netcool-probe-messagebus-kafka-prod-rb.yaml

  # Delete the role binding for all service accounts in the current namespace
  kubectl delete -f $scriptDir/$namespace-ibm-netcool-probe-messagebus-kafka-prod-rb.yaml -n $namespace

  # Clean up - delete the temporary yaml file.
  rm $scriptDir/$namespace-ibm-netcool-probe-messagebus-kafka-prod-rb.yaml
else
