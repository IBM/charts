...
if supports_scc; then
  ...
  oc adm policy add-scc-to-group ibm-netcool-probe-messagebus-kafka-prod-scc system:serviceaccounts:$namespace
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' $scriptDir/ibm-netcool-probe-messagebus-kafka-prod-rb.yaml > $scriptDir/$namespace-ibm-netcool-probe-messagebus-kafka-prod-rb.yaml

  # Create the role binding for all service accounts in the current namespace
  kubectl create -f $scriptDir/$namespace-ibm-netcool-probe-messagebus-kafka-prod-rb.yaml -n $namespace
