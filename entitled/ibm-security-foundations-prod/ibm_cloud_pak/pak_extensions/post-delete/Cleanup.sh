...
remove_podfin() {
  ...
  do
    ...
    kubectl delete $pod --wait=false 2>&1 | grep -v NotFound
  done
  ...
  do
     ...
     kubectl delete $pod --force=true --grace-period=0
  done
}
...
remove_crfin couchdata.isc.ibm.com

kubectl delete redissentinel -lapp.kubernetes.io/managed-by=couch-wrapper --wait=false
kubectl delete couchdbcluster -lapp.kubernetes.io/managed-by=couch-wrapper --wait=false
...
kubectl delete secret -n $NAMESPACE isc-jwt --ignore-not-found=true
...
# remove serviceability components
kubectl delete deploy cp-serviceability --ignore-not-found=true
kubectl delete serviceaccount cp-serviceability-sa --ignore-not-found=true
kubectl delete clusterrole cp-serviceability-role --ignore-not-found=true
kubectl delete clusterrolebinding cp-serviceability-rolebinding --ignore-not-found=true
