...
#################################################################
#
# Run this script as a cluster admin for each helm release.
#
# This script takes one argument; the helm release which the helm chart was installed to
#
# Example:
#     bash mustgather.sh myHelmReleaseName
#
...
if [ "$#" -lt 1 ]; then
  echo "Usage: bash mustgather.sh myHelmReleaseName"
  exit 1
fi
...
kubectl describe nodes > ${RELEASE_NAME}/kubectl-describe-nodes.log
...
if [ `which zip` ]; then
	zip -qr ${RELEASE_NAME}-$(date +%y%m%d%H%M%S).zip ${RELEASE_NAME}
else
