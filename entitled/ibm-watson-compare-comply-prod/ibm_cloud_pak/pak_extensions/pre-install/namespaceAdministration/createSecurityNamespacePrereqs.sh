...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group ibm-cnc-prod-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-cnc-prod-rb.yaml > $namespace-ibm-cnc-prod-rb.yaml
  ...
  kubectl create -f $namespace-ibm-cnc-prod-rb.yaml -n $namespace

  # Clean up - delete the temporary yaml file.
  rm $namespace-ibm-cnc-prod-rb.yaml
fi;