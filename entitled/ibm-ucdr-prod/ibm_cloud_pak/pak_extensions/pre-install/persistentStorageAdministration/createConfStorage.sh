...
sed -e 's/{{ NFSSERVER }}/'$nfsserver'/g' -e 's/{{ NFSPATH }}/'$nfspath'/g' $scriptDir/ibm-ucdr-prod-conf.yaml > $scriptDir/$$-ibm-ucdr-prod-conf.yaml


# Create the PV and PVC to hold the conf directory
echo "Creating Persistent Volume and Persistent Volume Claim from $scriptDir/ibm-ucdr-prod-conf.yaml template file"
kubectl apply -f $scriptDir/$$-ibm-ucdr-prod-conf.yaml

# Clean up - delete the temporary yaml file
rm -f $scriptDir/$$-ibm-ucdr-prod-conf.yaml
