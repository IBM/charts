...
if supports_psp; then
    ...
    sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-ucdr-prod-rb.yaml > $namespace-ibm-ucdr-prod-rb.yaml

    # Create the role binding for all service accounts in the current namespace
    kubectl create -f $namespace-ibm-ucdr-prod-rb.yaml -n $namespace
...
if supports_scc; then
    ...
    if command -v oc >/dev/null 2>&1 ; then
        ...
        oc adm policy add-scc-to-group ibm-ucdr-prod-scc system:serviceaccounts:$namespace
    else
