...
sed 's/{{ NAMESPACE }}/'$namespace'/g' ../../pre-install/namespaceAdministration/wa-pod-label-rb.yaml > $namespace-wa-pod-label-rb.yaml

# Delete the role binding for all service accounts in the current namespace
kubectl delete -f $namespace-wa-pod-label-rb.yaml -n $namespace
