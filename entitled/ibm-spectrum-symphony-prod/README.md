...
- To install a licensed version of IBM Spectrum Symphony, obtain an entitlement file for IBM Spectrum Symphony ([talk to an IBM representative for details](https://www.ibm.com/us-en/marketplace/analytics-workload-management)). Once you have the entitlement file, complete the following steps before the chart is deployed: 
...
The following table lists the configurable parameters of the ibm-spectrum-symphony-prod chart and their default values. Specify each parameter using the "--set key=value[,key=value]" argument to the "helm install --tls" command.
...
| cluster.clusterName              | Name of the cluster. If "", the cluster takes the name '*release_name*-ibm-spectrum-symphony-prod'. | `""` |
...
| master.entitlementSecretName |  Name of the secret created for IBM Spectrum Symphony entitlement (see [Before Installing the Chart](#before-installing-the-chart)). | `""` |
