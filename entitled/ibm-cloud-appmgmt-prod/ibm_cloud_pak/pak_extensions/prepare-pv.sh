...
You will apply these persistent storage definitions onto ICP via kubectl create -f ${DIR}/../yaml"
    ...
    echo "    --size0_ppc64le                           Install as size0 on ppc64le (minimum resource requirements)  - if omitted, specify each size in parameters"
    echo "    --size1_amd64                             Install as size1 on amd64 (standard resource requirements) - if omitted, specify each size in parameters"
    echo "    --size1_ppc64le                           Install as size1 on ppc64le (standard resource requirements) - if omitted, specify each size in parameters"
    echo
...
IDENTIFY_STORAGE_SIZE() {
  if [ $global_environmentSize == "size1_amd64" ] || [ $global_environmentSize == "size1_ppc64le" ] ; then
