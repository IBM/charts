...
WRITE_VALUES() {
    ## move from IDENFIFY_ICP_ENVIRONMENT()
...
HTTPS() {
    if [[ $https == "true" || $advanced == "true" ]]; then
        server_secret=${release_name}-ingress-tls
        client_secret=${release_name}-ingress-client
        certificate_archive=${release_name}-ingress-artifacts
