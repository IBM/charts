...
3. {{ if .Release.IsUpgrade }}(Optional) {{ end }}OIDC registration with IBM Cloud Private is required to be able to login to IBM Cloud App Management's UI. As an IBM Cloud Private user with the Cluster Administrator role, run the following kubectl command:
...
{{ if .Release.IsUpgrade }}(Optional) {{ end }}Policy registration with IBM Cloud Private is required to allow IBM Cloud App Management's services to be able to access other services. As an IBM Cloud Private user with the Cluster Administrator role, run the following kubectl command:
