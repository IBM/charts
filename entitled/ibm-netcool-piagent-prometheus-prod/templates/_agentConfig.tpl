...
RestCommand_service_cpu_metrics.json: |
 {
   ...
   "method" : "GET",
   "url" : "https://{{ "{{" }}prometheus_endpoint{{ "}}" }}/api/v1/query_range?query=sum(rate(container_cpu_usage_seconds_total{{ "{{" }}container_labels{{ "}}" }}[5m])) by (instance)&start={{ "{{" }}start_time{{ "}}" }}&end={{ "{{" }}end_time{{ "}}" }}&step={{ "{{" }}step_time{{ "}}" }}",
   "headers" : { },
   "parameters" : { },
   "commandInputs" : [ {
     "id" : "query_string",
