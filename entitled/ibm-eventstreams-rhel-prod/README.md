...
```
helm upgrade --reuse-values --set kafka.configMapName=<configmap_name> <release_name> ibm-eventstreams-rhel-prod --tls
```
...
```
helm install --name <release_name> --namespace=<namespace_name> --set license=accept local-charts/ibm-eventstreams-rhel-prod --tls
```
...
The following tables list the configurable parameters of the `ibm-eventstreams-rhel-prod` chart and their default values.
