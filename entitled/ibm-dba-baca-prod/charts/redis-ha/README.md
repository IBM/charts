...
- Change the values for the `reposittory` and `tag` under `image` and tag to match your existing environment.  For example:
```
image:
  repository: mycluster.icp:8500/sp/redis
  tag: latest
  pullPolicy: Always
```
- Change the `existingClaim` to Content Analyzer data pvc created by running the `init_deployments.sh`.  By default, it's `sp-data-pvc`
## Upgrading the Chart
...
```bash
$ helm install . -f values.yaml --name baca-redis --namespace <namespace> --tls
```
...
## Post installation for Content Analyzer

- Before installing the rest of Content Analyzer's component:
    1) Navigate to `stable/ibm-dba-baca-prod`
    2) Edit the `requirements.yaml`, and comment out or remove the redis section
    3) Navigate to `stable/ibm-dba-baca-prod/charts`.
    4) Move the `redis` directory out of the `stable/ibm-dba-baca-prod/charts` to a backup location.
