...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    ...
    oc delete -f "${DIR}/../../pre-install/namespaceAdministration/ibm-db2warehouse-role.yaml" -n ${namespace}
    oc delete -f "${DIR}/../../pre-install/namespaceAdministration/ibm-db2warehouse-rb.yaml" -n ${namespace}
    oc adm policy remove-scc-from-user db2wh-scc  system:serviceaccount:${namespace}:db2u
  else
...
#  sed 's//'$namespace'/g' "${DIR}/../../pre-install/namespaceAdministration/ibm-db2warehouse-rb.yaml" > $namespace-ibm-db2warehouse-rb.yaml

  # Delete the role binding for all service accounts in the current namespace
#  kubectl delete -f "${DIR}/$namespace-ibm-db2warehouse-rb.yaml" -n $namespace
