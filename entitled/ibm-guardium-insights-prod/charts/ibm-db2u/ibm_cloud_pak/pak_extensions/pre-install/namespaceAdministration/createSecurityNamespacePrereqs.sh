...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    if [[ -z `oc get sa db2u --ignore-not-found` ]] ; then
       oc create serviceaccount db2u -n ${namespace}
    else
       echo "Service account 'db2u' already exists."
    fi
    if [[ -z `oc get role ibm-db2warehouse-role --ignore-not-found` ]] ; then
       oc create -f "${DIR}/ibm-db2warehouse-role.yaml" -n ${namespace}
    else
       echo "Role 'ibm-db2warehouse-role' already exists."
    fi
    if [[ -z `oc get rolebinding ibm-db2warehouse-rolebinding --ignore-not-found` ]] ; then
    oc create -f "${DIR}/ibm-db2warehouse-rb.yaml" -n ${namespace}
    else
       echo "Role binding 'ibm-db2warehouse-rolebinding' already exists."
    fi
