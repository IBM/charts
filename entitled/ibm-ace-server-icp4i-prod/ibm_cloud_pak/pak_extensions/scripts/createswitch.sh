...
#    helm install --name <ReleaseName> ibm-ace-server-icp4i-prod --tls --set license=accept \
...
kubectl patch deployment $RELEASE-ibm-ace-server-icp4i-prod -n $NAMESPACE -p "{\"spec\":{\"template\":{\"metadata\":{\"labels\":{\"date\":\"`date +'%s'`\"}}}}}"
