...
if supports_scc; then
  ...
  oc adm policy add-scc-to-group ibm-netcool-gateway-cem-prod-scc system:serviceaccounts:$namespace
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' $scriptDir/ibm-netcool-gateway-cem-prod-rb.yaml > $scriptDir/$namespace-ibm-netcool-gateway-cem-prod-rb.yaml

  # Create the role binding for all service accounts in the current namespace
  kubectl create -f $scriptDir/$namespace-ibm-netcool-gateway-cem-prod-rb.yaml -n $namespace
