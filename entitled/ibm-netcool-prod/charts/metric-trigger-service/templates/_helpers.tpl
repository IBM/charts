...
*/ -}}
{{- define "metric-trigger-service.image.url" -}}
  {{- $params := . -}}
  {{- $root := first $params -}}
...
{{- define "metric-trigger-service.container.security.context" -}}
securityContext:
  privileged: false
...
{{- define "metric-trigger-service.spec.security.context" -}}
hostNetwork: false
hostPID: false
hostIPC: false
securityContext:
...
*/ -}}
{{- define "metric-trigger-service.getRegistryRelease" -}}
  {{- $root := index . 0 -}}
...
{{- define "metric-trigger-service.getKafkaRelease" -}}
  {{- $root := index . 0 -}}
  {{- $userDefinedRelease := index . 1 -}}
