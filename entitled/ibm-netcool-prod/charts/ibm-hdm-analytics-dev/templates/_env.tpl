...
- name: SEAS_EXEC_OPTS
  value: "-Xdisableexplicitgc -Xgcthreads1 -Xms1024M -XX:MaxDirectMemorySize=512M"
- name: MAD_EXECUTOR_MEMORY
  value: "1024M"
- name: MAD_DRIVER_MEMORY
  value: "512M"
- name: MAD_DRIVER_CORES
  value: "1"
- name: MAD_DRIVER_OPTS
  value: "-Xdisableexplicitgc -Xgcthreads1 -Xms256M"
- name: MAD_EXEC_OPTS
  value: "-Xdisableexplicitgc -Xgcthreads1 -Xms1024M -XX:MaxDirectMemorySize=512M"
{{- else if eq .Values.global.environmentSize "size1" }}
...
{{- define "eventanalytics.common.setupjob.retry" -}}
- name: SETUP_RETRY_ONFAILURE
  value: "1"
- name: SETUP_RETRY_BACKOFF
  value: "30"
- name: SETUP_RETRY_NUMBER
  value: "10"
{{- end -}}
...
{{ define "eventanalytics.NOI_DASH_AUTH_SERVLET_URL" -}}
  {{- if .Values.common.services.noiDashAuthUrl -}}
    {{- .Values.common.services.noiDashAuthUrl -}}
  {{- else if not .Values.global.hybrid.disabled -}}
    {{- .Values.global.hybrid.dash.url -}}
  {{- else -}}
...
- name: SCHEDULINGUI_URL
  value: 'https://dummyUrl'
- name: UAG_URL
{{- if .Values.common.services.uagUrl }}
  value: "{{ .Values.common.services.uagUrl }}"
{{- else }}
  value: "http://{{ .Release.Name }}-ibm-cem-cem-users.{{ .Release.Namespace }}.svc:6002/"
{{- end }}
- name: NOI_DASH_AUTH_SERVLET_URL
  value: {{ include "eventanalytics.NOI_DASH_AUTH_SERVLET_URL" . | quote }}
{{- end -}}
...
- name: COUCHDB_URL
  value: '{{ .Values.couchdb.scheme }}://{{ .Release.Name }}-couchdb:{{ .Values.couchdb.port }}/'
