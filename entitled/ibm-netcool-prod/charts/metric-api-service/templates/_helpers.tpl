...
*/ -}}
{{- define "metric-api-service.cluster.fqdn" -}}
...
{{- define "metric-api-service.ingress.globalhost" -}}
  {{- $root := index . 0 -}}
  {{- $prefix := index . 1 -}}
  {{- $ingressGlobal := $root.Values.global.ingress }}
  {{- $ingressDomain := include "metric-api-service.cluster.fqdn" $root -}}
...
{{- define "metric-api-service.ingress.host" -}}
  {{- $ingressComp := .Values.ingress }}

  {{- include "metric-api-service.ingress.globalhost" (list . $ingressComp.prefix) -}}
{{- end -}}

{{- define "metric-api-service.ingress.baseurl" -}}
  {{- $ingressGlobal := .Values.global.ingress }}
  {{- $ingressComp := .Values.ingress }}
  {{- $ingressHost := include "metric-api-service.ingress.host" . -}}
...
*/ -}}
{{- define "metric-api-service.image.url" -}}
  {{- $params := . -}}
  {{- $root := first $params -}}
...
{{- define "metric-api-service.container.security.context" -}}
securityContext:
  privileged: false
...
{{- define "metric-api-service.getCemusersUrl" -}}
  {{- $root := index . 0 -}}
  {{- $varName := index . 1 -}}
...
{{- define "metric-api-service.spec.security.context" -}}
hostNetwork: false
hostPID: false
hostIPC: false
securityContext:
