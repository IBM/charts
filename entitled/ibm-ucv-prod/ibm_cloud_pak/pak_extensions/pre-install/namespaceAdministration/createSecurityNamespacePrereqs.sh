...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group ibm-ucv-prod-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-ucv-prod-rb.yaml > $namespace-ibm-ucv-prod-rb.yaml
  ...
  kubectl create -f $namespace-ibm-ucv-prod-rb.yaml -n $namespace
