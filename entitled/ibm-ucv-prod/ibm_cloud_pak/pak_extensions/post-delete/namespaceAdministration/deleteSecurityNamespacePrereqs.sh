...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy remove-scc-from-group ibm-ucv-prod-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' ../../pre-install/namespaceAdministration/ibm-ucv-prod-rb.yaml > $namespace-ibm-ucv-prod-rb.yaml

  # Delete the role binding for all service accounts in the current namespace
  kubectl delete -f $namespace-ibm-ucv-prod-rb.yaml -n $namespace

  # Clean up - delete the temporary yaml file.
  rm $namespace-ibm-ucv-prod-rb.yaml
fi