...
sed 's/{{ NAMESPACE }}/'$namespace'/g' $scriptDir/ibm-netcool-probe-syslogd-prod-rb.yaml > $scriptDir/$namespace-ibm-netcool-probe-syslogd-prod-rb.yaml

# Create the role binding for all service accounts in the current namespace
kubectl create -f $scriptDir/$namespace-ibm-netcool-probe-syslogd-prod-rb.yaml -n $namespace
